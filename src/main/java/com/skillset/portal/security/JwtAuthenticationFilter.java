    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (StringUtils.hasText(jwt) && tokenProvider.validateToken(jwt)) {
            ...
            if (dbToken.isPresent() && dbToken.get().getExpiryDate().isAfter(LocalDateTime.now())) {
                ...
                if (employeeOpt.isPresent() && employeeOpt.get().getRole() != null) {

                    String roleName = employeeOpt.get().getRole().getRoleName().toUpperCase();
                    ...
                    logger.warn("Employee '" + email + "' has no role assigned or was not found; request will be treated as unauthenticated.");
